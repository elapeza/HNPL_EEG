...
PowerSupply = 5;%default is 3.3V, but we set 5 V to be same as OTBioLab+
...
for nChild = 1:length(abs.Device.Channels.Adapter)
    ...
    for nChan=1:length(Channel)
        ...
        %Changed by shintaro
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        elseif contains(Description,'64')  && contains(ChannelAtt.ID,'GR04MM1305')
            Adapter(startIndex+idx+1) = 4;
        elseif contains(Description,'64')  && contains(ChannelAtt.ID,'EEG')
            Adapter(startIndex+idx+1) = 4.5;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if contains(abs.Device.Attributes.Name, 'QUATTROCENTO')
            ...
            %Changed by shintaro
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            if contains(Adpatidx,'MULTIPLE IN') && contains(Description,'EMG')
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Added by Shintaro
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
signal.eegdata = data(Adapter == 4.5,:);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Changed by shintaro
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
signal.nChan=sum(Adapter == 3 | Adapter == 4);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:signal.ngrid
    ...
    signal.eeggridname{i} = Grid3{idx};
